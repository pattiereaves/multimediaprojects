			...
			*/
			...
			if( stripos($element->getAttribute( 'class' ), 'subtitle') ) {
				...
				if( is_numeric($rich_media->filename) ) {
					switch( $rich_media->filetype ) {
						case 'video' :
							import_video( $rich_media->filename );
							break;
						default :
...
/**
 * Import a video from the BDN database based on its post ID number
 *
...
function import_video(&$filename) {
	...
	if(!is_numeric($filename)) 
		return $filename; //proceed without checking for the video file

	$url = 'https://bangordailynews.com/embedvideo/?video_id='.$filename;	                  
	
	$embed_video = file_get_contents( $url );
	if(empty($embed_video)) {
		die("Could not find video {$filename}. \n");
	} else {
		preg_match( "#(?'url'http:\\\/\\\/watchvideo\.bangordailynews\.com\S*m4v)#", $embed_video, $matches );
		if($matches['url']) {
			$final = stripslashes( $matches['url'] );
			echo "Included video {$final}. \n";
			$filename = $final;
		} else {
