			...
			*/
			...
			if( stripos($paragraph->getAttribute( 'class' ), 'subtitle') ) {
				...
				if(is_numeric($rich_media->filename)) {
					bdn_input_merlin_photo($rich_media->filename); //see merlin.php
					$rich_media->filename = $rich_media->filename . ".jpg";
					echo "Merlin ID detected. \n";
				} elseif(is_numeric($rich_media->poster)) {
					//@todo there is a more elegant way than to do this twice.
					bdn_input_merlin_photo($rich_media->poster); //see merlin.php
					$rich_media->poster = $rich_media->poster . ".jpg";
					echo "Merlin ID detected. \n";
				}
